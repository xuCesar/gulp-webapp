...
gulp.task('rev', ['html', 'imagesRev'], function() {
    gulp.src(['rev/**/*.json', 'dist/**/*.*', '!dist/images/**/*.*'])   //- 读取 rev-manifest.json 文件,根据json的内容进行重命名替换
        .pipe($.revCollector({
            replaceReved: true,
            dirReplacements: {
                // 'css': '/dist/styles',
...
gulp.task('revBuild', ['lint', 'rev', 'fonts', 'extras'], () => {
